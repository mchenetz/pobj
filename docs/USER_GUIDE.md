...
This guide covers end-to-end usage of `pxobj`: installation, configuration, TLS/cert handling, COSI provisioning, S3 client access, scaling, operations, and troubleshooting.
...
```bash
go build ./...
make docker-build IMAGE=ghcr.io/<your-org>/pxobj:<tag>
```
...
```bash
helm upgrade --install pxobj ./charts/pxobj \
...
helm upgrade --install pxobj ./charts/pxobj \
  --namespace pxobj-system --create-namespace \
  --set image.repository=ghcr.io/<your-org>/pxobj \
  --set image.tag=<tag> \
...
helm upgrade --install pxobj ./charts/pxobj \
  ...
  --set objectService.issuerRefName=pxobj-ca-issuer
```
...
If needed, update image in `deploy/operator.yaml` and `PXOBJ_IMAGE` env.
...
```bash
kubectl -n pxobj-system rollout status statefulset/pxobj
kubectl -n pxobj-system rollout status deploy/pxobj-cosi
```
...
```bash
kubectl -n pxobj-system patch objectservice pxobj --type merge -p '{"spec":{"replicas":5}}'
kubectl -n pxobj-system rollout status statefulset/pxobj
```
...
Order:
1. Push new image.
2. Update operator deployment image and `PXOBJ_IMAGE` env.
3. Let operator roll.
...
```bash
kubectl -n pxobj-system set image deploy/pxobj-operator operator=ghcr.io/<org>/pxobj:<tag>
kubectl -n pxobj-system set env deploy/pxobj-operator PXOBJ_IMAGE=ghcr.io/<org>/pxobj:<tag>
kubectl -n pxobj-system rollout status deploy/pxobj-operator
kubectl -n pxobj-system rollout status statefulset/pxobj
kubectl -n pxobj-system rollout status deploy/pxobj-cosi
```
...
```bash
kubectl -n pxobj-system logs deploy/pxobj-operator
kubectl -n pxobj-system get objectservice pxobj -o yaml
```
...
kubectl -n pxobj-system logs deploy/pxobj-cosi
```
...
```bash
kubectl -n pxobj-system get secret pxobj-tls -o yaml
```
...
```bash
kubectl -n pxobj-system logs statefulset/pxobj -c objectd --tail=200
```
...
kubectl -n pxobj-system delete objectservice pxobj --ignore-not-found
...
kubectl delete -f config/crd/bases/pxobj.io_objectservices.yaml --ignore-not-found
```
